...
## 推荐定位（重要）
推荐将本 Skill 作为“工具库”使用，而不是强绑定单一分析逻辑：

- Skill 提供工具能力：
  - `tool-venue-profile`：会议规则/评分维度解析
  - `tool-parse-paper`：PDF/Markdown 结构化提取
  - `tool-format-template`：分析结果模板契约
  - `tool-format-student-pack`：标准化输出 3 份研究生可读文档
- 上层 Agent 负责语义分析：
  - 读论文
  - 识别 gaps
  - 写 rebuttal
...
一句话：代码负责“稳定提取 + 标准输出”，Agent 负责“阅读理解 + 判断生成”。

## Manifest（Agent 可执行清单）
Skill 同级提供 `manifest.json`，用于告诉 Agent：
- 可调用工具有哪些
- 每个工具输入输出是什么
- 哪些命令内部会调用 LLM（`internal_llm: true`）
- 推荐高层流程（`agent-first-review` / `full-review`）

建议 Agent 先读取：
1. `agent-paper-reviewers-skill/manifest.json`
2. `agent-paper-reviewers-skill/flow_config.yaml`
...
## 运行时说明（纯 Skill）
- 当前主流程不依赖任何 provider 抽象层。
- 会议规则解析走本地 `data/venue_rules/*`。
- 未知会议处理：本地 `_fallback` + executor 自举规则草案。
...
- `runtime_context.json`：运行时能力上下文（当前固定 `local_skill_tools_only`）。
